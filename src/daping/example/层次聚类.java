...
public class 层次聚类 {
    ...
    static final Distance<CharSequence> EDIT_DISTANCE
        = new EditDistance(false);// indicates that transposition is not permitted as an edit operation. 


    public static void main(String[] args) {
        // parse out input set
        Set<String> inputSet = new HashSet<String>();
        String texts[] = {"aa, aaa, aaaaa, bbb, bbbb", "3"};
        for (String s : texts[0].split(",")) 
            inputSet.add(s);

        // set up max distance: maxDistance Maximum distance between clusters that can be linked.
        int maxDistance = args.length == 1
            ? Integer.MAX_VALUE
            : Integer.valueOf(texts[1]);
        ...
        // Single-Link Clusterer ,tends to create highly separated clusters
        HierarchicalClusterer<String> slClusterer 
            = new SingleLinkClusterer<String>(maxDistance,
                                              EDIT_DISTANCE);

        // Complete-Link Clusterer， tends to create more tightly centered clusters
