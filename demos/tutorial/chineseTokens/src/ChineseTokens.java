...
public class ChineseTokens {
    ...
    public ChineseTokens(String[] args) {
        ...
        String daping_args[]={"E:/JAVAProjects/lingpipe-4.1.0/demos/data/chinese word segmentation/", "cityu", "hk", "cityu.out", "Big5_HKSCS", "5", "5.0", "5000", "256", "0.0", "0.0"};
        
        mDataDir = new File(daping_args[0]);
        mTrainingCorpusName = daping_args[1];
        mTestCorpusName = daping_args[2];
        mOutputFile = new File(mDataDir,daping_args[3]+".segments");
        mKnownToksFile = new File(mDataDir,daping_args[3]+".knownWords");
        mCharEncoding = daping_args[4];
        mMaxNGram = Integer.valueOf(daping_args[5]);
        mLambdaFactor = Double.valueOf(daping_args[6]);
        mNumChars = Integer.valueOf(daping_args[7]);
        mMaxNBest = Integer.valueOf(daping_args[8]);
        mContinueWeight = Double.valueOf(daping_args[9]);
        mBreakWeight = Double.valueOf(daping_args[10]);
